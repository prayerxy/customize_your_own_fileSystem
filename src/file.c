...
static int XCraft_ext_file_get_block(struct inode *inode,
									 ...
									 int create)
{
	pr_debug("xcraft: beign ext_file_get_block\n");
	struct super_block *sb = inode->i_sb;
...
#endif
{
	pr_debug("xcraft: ext_write_begin is doing!\n");
	...
	pr_debug("xcraft: ext_write_begin is done! err is %d\n", err);
	return err;
}
...
{
	pr_debug("xcraft: ext_write_end is doing\n");
	...
	mark_inode_dirty(inode);
	pr_debug("xcraft: ext_write_end is done, ret is %d\n", ret);
end:
	return ret;
}
